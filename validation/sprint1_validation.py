...
protein_go_map_full = parse_goa_human(goa_file, evidence_filter="manual", aspects={"P", "F", "C"})
...
domain_architectures = parser_obj.parse_protein2ipr_file(
    interpro_file,
    protein_filter=set(protein_ids_subset)
)
...
supra_pairs = sum(1 for m in metadata_supra.values() if m.domain_type == DomainType.SUPRA_PAIR)
supra_triples = sum(1 for m in metadata_supra.values() if m.domain_type == DomainType.SUPRA_TRIPLE)
supra_total = supra_pairs + supra_triples

print(f"\n  Domain composition:")
print(f"    Single domains:         {single_count:,} ({single_count/len(metadata_supra)*100:.1f}%)")
print(f"    Supra-domain pairs:     {supra_pairs:,} ({supra_pairs/len(metadata_supra)*100:.1f}%)")
print(f"    Supra-domain triples:   {supra_triples:,} ({supra_triples/len(metadata_supra)*100:.1f}%)")
print(f"    Total supra-domains:    {supra_total:,} ({supra_total/len(metadata_supra)*100:.1f}%)")
...
print(f"  Additional statistical tests:  +{tests_increase:,} ({tests_increase_pct:.1f}% increase)")
print(f"\n  Matrix density:")
...
multi_domain_proteins = sum(1 for arch in domain_architectures.values() if len(arch.single_domains) >= 2)
multi_domain_pct = (multi_domain_proteins / len(domain_architectures)) * 100

print(f"\n  Multi-domain protein coverage:")
print(f"    Proteins with 2+ domains:      {multi_domain_proteins:,} ({multi_domain_pct:.1f}%)")
