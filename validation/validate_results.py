...
def load_dcgo_predictions(predictions_file: Path) -> pd.DataFrame:
    ...
    logger.info(f"  p-value range: {df['p_value'].min():.2e} to {df['p_value'].max():.2e}")
    logger.info(f"  Hyper score range: {df['hyper_score'].min():.2f} to {df['hyper_score'].max():.2f}")
...
def load_domain_architectures(protein2ipr_file: Path) -> tuple:
    ...
    for domain, proteins in domain_proteins.items():
        ...
        domain_stats[domain] = {
            ...
            'fraction_single': len(single_domain_proteins) / len(proteins) if proteins else 0
        }
...
def calculate_metrics(predictions: pd.DataFrame, gold_standard: dict, threshold_name: str) -> dict:
...
def analyze_novel_predictions(
    ...
    novel_df = predictions[
        predictions.apply(lambda row: (row['domain'], row['go_term']) in novel_pairs, axis=1)
    ].copy()
...
def analyze_domain_architecture_contribution(
    ...
    if not df.empty:
        ...
        logger.info(f"  Median proteins per association: {df['total_proteins'].median():.0f}")
...
def create_visualizations(
    ...
    ax.set_title('Precision vs Recall at Different Thresholds', fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.3)

    # F1 Score by number of predictions
    ax = axes[0, 1]
    ax.scatter(metrics_df['n_predictions'], metrics_df['f1_score'], alpha=0.6, s=100, c=metrics_df['precision'], cmap='viridis')
    ...
    ax = axes[1, 0]
    ax.scatter(metrics_df['false_positives'], metrics_df['true_positives'], alpha=0.6, s=100)
    ...
    if not architecture_df.empty:
        ax = axes[1, 1]
        ax.hist(architecture_df['fraction_single'], bins=20, alpha=0.7, edgecolor='black')
        ax.axvline(architecture_df['fraction_single'].median(), color='red', linestyle='--', linewidth=2, label=f'Median: {architecture_df["fraction_single"].median():.2%}')
        ax.set_xlabel('Fraction Single-Domain Proteins', fontsize=12)
        ax.set_ylabel('Number of Associations', fontsize=12)
        ax.set_title('Single-Domain Protein Contribution', fontsize=14, fontweight='bold')
        ax.legend()
...
def main():
    ...
    predictions_file = Path("results_no_true_path/domain_go_associations_significant.tsv")
    ...
    logger.info(f"✓ Overlap: {len(overlap):,} ({len(overlap)/len(pred_set)*100:.2f}% of predictions)")
    logger.info(f"  Novel predictions: {len(novel):,} ({len(novel)/len(pred_set)*100:.2f}%)")
    logger.info(f"  Missed gold standard: {len(missed):,} ({len(missed)/len(gold_set)*100:.2f}%)")
    ...
    logger.info(f"✓ Best F1 score: {best_row['f1_score']:.4f} at {best_row['threshold']}")
    ...
    logger.info(f"✓ Saved novel predictions: {output_dir / 'novel_predictions_top100.tsv'}")
    ...
    if protein2ipr_file.exists():
        ...
        architecture_df = analyze_domain_architecture_contribution(predictions, gold_standard, domain_stats)

        if not architecture_df.empty:
            architecture_df.to_csv(output_dir / "architecture_contribution.tsv", sep='\t', index=False)
            logger.info(f"✓ Saved architecture analysis: {output_dir / 'architecture_contribution.tsv'}")
    else:
    ...
    logger.info("=" * 70)
    logger.info("Summary:")
    logger.info(f"  Overlap with gold standard: {len(overlap):,} / {len(pred_set):,} ({len(overlap)/len(pred_set)*100:.2f}%)")
    logger.info(f"  Best F1 score: {best_row['f1_score']:.4f}")
    logger.info(f"  Novel high-confidence predictions: {len(novel):,}")
    if not architecture_df.empty:
        logger.info(f"  Avg single-domain contribution: {architecture_df['fraction_single'].mean():.2%}")
    logger.info("")
