...
class DataAcquisition:
    ...
    async def download_all_datasets_async(self) -> Dict[str, Path]:
        ...
        for source_name, url in self.config.DATASOURCES.items():
            ...
            if filepath.exists() and filepath.stat().st_size > 0:
                logger.info(f"File {filepath} already exists ({filepath.stat().st_size:,} bytes), skipping")
        ...
        if download_tasks:
            ...
            for i, result in enumerate(results):
                ...
                source_name = task.get_name() if hasattr(task, 'get_name') else f"task_{i}"
                
                if isinstance(result, Exception):
                    logger.error(f"Failed to download {source_name}: {result}")
                    if source_name in ['uniprot_sprot', 'goa_annotations', 'go_ontology']:
                        raise DataAcquisitionError(f"Critical dataset {source_name} failed: {result}")
                elif isinstance(result, tuple):
                    name, path = result
                    downloaded_files[name] = path
        
        logger.info(f"Async dataset download completed. Downloaded {len(downloaded_files)} files")
        return downloaded_files
    
    async def _download_async(self, source_name: str, url: str, filepath: Path) -> Tuple[str, Path]:
    ...
    def download_with_progress(
        ...
        if verify_checksum:
            if not self._verify_checksum(filepath, verify_checksum):
                ...
                raise DataAcquisitionError(f"Checksum verification failed for {filepath}")
    ...
    def _verify_checksum(self, filepath: Path, expected_md5: str) -> bool:
        ...
        try:
            ...
            if matches:
                ...
                logger.error(f"Checksum mismatch for {filepath}: expected {expected_md5}, got {calculated_md5}")
    ...
    def download_all_datasets(self) -> Dict[str, Path]:
        ...
        for source_name, url in self.config.DATASOURCES.items():
            try:
                ...
                if filepath.exists() and filepath.stat().st_size > 0:
                    logger.info(f"File {filepath} already exists ({filepath.stat().st_size:,} bytes), skipping download")
                else:
                    # Download the file
        ...
        logger.info(f"Dataset download completed. Downloaded {len(downloaded_files)} files")
        return downloaded_files
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=1) as executor:
                interpro_script = await loop.run_in_executor(
                    executor, self._extract_interproscan_sync, interpro_archive, extract_dir
                )
    ...
    def _extract_interproscan_sync(self, interpro_archive: Path, extract_dir: Path) -> Path:
    ...
    def setup_interproscan(self, interpro_archive: Path) -> Path:
        ...
        # Extract the archive
        try:
            interpro_script = self._extract_interproscan_sync(interpro_archive, extract_dir)
            self._configure_interproscan(interpro_script)
            return interpro_script
            
        except Exception as e:
            raise DataAcquisitionError(f"Failed to extract InterProScan archive: {e}") from e
    ...
    async def async_download_session(self):
        ...
        try:
            ...
            session_duration = time.time() - session_start
            logger.info(f"Async download session completed in {session_duration:.2f} seconds")
    ...
    async def verify_all_downloads_async(self) -> Dict[str, bool]:
        ...
        passed = sum(verification_results.values())
        total = len(verification_results)
        logger.info(f"Async dataset verification completed: {passed}/{total} files passed")
        
        return verification_results
    
    async def _verify_file_async(self, source_name: str, filepath: Path) -> Tuple[str, bool]:
    ...
    async def download_with_retry_async(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.info(f"Successfully downloaded {filepath} on attempt {attempt + 1}")
                return
        ...
        raise DataAcquisitionError(f"Failed to download {url} after {max_retries + 1} attempts: {last_exception}")
    ...
    def get_progress_summary(self) -> Dict[str, Dict[str, Any]]:
        ...
        for source_name, progress in self._download_progress.items():
            summary[source_name] = {
                ...
                'total_size_mb': (progress.total_size / (1024 * 1024)) if progress.total_size else None,
