...
class DomainArchitectureScanner:
    ...
    def _validate_interproscan(self) -> None:
        ...
        if not self.interproscan_path.exists():
            raise FileNotFoundError(
                f"InterProScan not found at {self.interproscan_path}"
            )

        if not self.interproscan_path.is_file():
            raise ValueError(
                f"InterProScan path is not a file: {self.interproscan_path}"
            )

        if not self.interproscan_path.stat().st_mode & 0o111:
            raise PermissionError(
                f"InterProScan is not executable: {self.interproscan_path}"
            )
                ...
                try:
                    with open_func(seq_file) as infile:
                        for record in SeqIO.parse(infile, "fasta"):
                            ...
                            record.id = clean_id
                            record.description = (
                                ""  # Remove description to avoid parsing issues
                            )
                            ...
                            if max_sequences and sequence_count >= max_sequences:
                                logger.info(
                                    f"Reached sequence limit of {max_sequences}"
                                )
                                break
    ...
    def _validate_sequence(self, record, clean_id: str) -> bool:
        ...
        if invalid_chars:
            logger.debug(
                f"Sequence {clean_id} contains invalid characters: {invalid_chars}"
            )
            return False
        ...
        if ambiguous_count / len(seq_str) > 0.5:
            logger.debug(
                f"Sequence {clean_id} has too many ambiguous residues ({ambiguous_count}/{len(seq_str)})"
            )
            return False
    ...
    def parse_interpro_output(
        self, tsv_file: Path, applications_filter: Optional[Set[str]] = None
    ) -> Dict[str, List[str]]:
        """
        ...
        logger.info(
            f"Found {len(filtered_hits)} hits from {applications_filter} across {filtered_hits['protein_id'].nunique()} proteins"
        )
    ...
    def _parse_domain_hits(self, df: pd.DataFrame) -> Dict[str, List[DomainHit]]:
        ...
        for _, row in df.iterrows():
            ...
            hit = DomainHit(
                ...
                if pd.notna(row["interpro_id"])
                else None,
                interpro_desc=row["interpro_desc"]
                if pd.notna(row["interpro_desc"])
                else None,
                go_annotations=row["go_annotations"]
                if pd.notna(row["go_annotations"])
                else None,
            )
    ...
    def _create_domain_architecture_map(
        self, domain_hits: Dict[str, List[DomainHit]]
    ) -> Dict[str, List[str]]:
        """
        ...
        logger.info(
            f"Created domain architectures for {len(protein_domains)} proteins:"
        )

        # Log statistics
        total_single = sum(
            len([d for d in domains if "," not in d])
            for domains in protein_domains.values()
        )
        total_supra = sum(
            len([d for d in domains if "," in d])
            for domains in protein_domains.values()
        )
    ...
    def _generate_supra_domains(
        self, domain_list: List[str], max_length: int = 3
    ) -> List[str]:
        """
    ...
    def export_domain_architectures(
        self, architectures: Dict[str, DomainArchitecture], output_file: Path
    ) -> None:
        ...
        for protein_id, arch in architectures.items():
            export_data[protein_id] = {
                ...
                "domain_positions": {
                    k: list(v) for k, v in arch.domain_positions.items()
                },
...
def main():
    ...
    parser.add_argument(
        "--interproscan", required=True, help="Path to InterProScan executable"
    )
    ...
    with DomainArchitectureScanner(
        interproscan_path=Path(args.interproscan), num_cores=args.cores
    ) as scanner:
        # Run InterProScan
        results = scanner.run_interproscan_batch(
            input_fasta=Path(args.input), output_dir=Path(args.output_dir)
        )
            ...
            json.dump(
                {
                    ...
                    "proteins_with_domains": len(
                        [p for p, d in domain_map.items() if d]
                    ),
