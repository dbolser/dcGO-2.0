...
class dcGOPipeline:
    ...
    def __init__(
        ...
        self.checkpoint_file = checkpoint_file or (
            self.config.LOGS_DIR / "pipeline_checkpoint.json"
        )
    ...
    def save_checkpoint(
        self, stage: PipelineStage, parameters: Dict[str, Any], **metadata: Any
    ) -> None:
        """
    ...
    def load_checkpoint(self) -> Optional[PipelineCheckpoint]:
        ...
        try:
            ...
            logger.info(
                f"Loaded checkpoint from stage {checkpoint.stage.name} "
    ...
    async def initialize_components(self, parameters: Dict[str, Any]) -> None:
        ...
        await asyncio.gather(
            init_data_acquisition(), init_database(), return_exceptions=True
        )
    ...
    async def run_data_acquisition(self, parameters: Dict[str, Any]) -> Dict[str, Path]:
        ...
        datasets_to_download = parameters.get(
            "datasets", list(self.config.DATASOURCES.keys())
        )
        ...
        try:
            ...
            if self.enable_async:
                ...
                with ThreadPoolExecutor(
                    max_workers=min(len(datasets_to_download), 3)
                ) as executor:
                    download_futures = []
                    ...
                    for dataset, future in download_futures:
                        try:
                            ...
                            logger.info(
                                f"Successfully downloaded {dataset}: {file_path}"
                            )
                        except Exception as e:
                            logger.error(f"Failed to download {dataset}: {e}")
                            raise PipelineError(
                                f"Data acquisition failed for {dataset}"
                            ) from e
            else:
                # Sequential download
                downloaded_files = {}
            ...
            logger.info(
                f"Data acquisition completed successfully. Downloaded {len(downloaded_files)} datasets."
            )
    ...
    async def run_domain_scanning(
        self, parameters: Dict[str, Any], data_files: Dict[str, Path]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not interpro_file or not interpro_file.exists():
                raise PipelineError(
                    "InterPro mappings file (protein2ipr.dat) not available"
                )
            ...
            parser = DomainAnnotationParser(
                max_supra_domain_length=parameters.get(
                    "max_supra_domain_length", self.config.MAX_SUPRA_DOMAIN_LENGTH
                ),
            ...
            logger.info(
                "Parsing protein2ipr.dat file (this may take several minutes)..."
            )
    ...
    async def run_ontology_processing(
        self, parameters: Dict[str, Any], data_files: Dict[str, Path]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.ontology_processor = await asyncio.to_thread(
                OntologyProcessor, go_ontology_file
            )

            # Parse GOA annotations with evidence code filtering
            evidence_filter = parameters.get(
                "evidence_filter", self.config.processing.evidence_filter
            )
            ...
            logger.info(
                f"Ontology processing completed. "
    ...
    async def run_statistical_inference(
        ...
        try:
            ...
            stats_params = {
                "fdr_threshold": parameters.get(
                    "fdr_threshold", self.config.FDR_THRESHOLD
                ),
                "min_proteins": parameters.get(
                    "min_proteins_per_association",
                    self.config.MIN_PROTEINS_PER_ASSOCIATION,
                ),
                "max_supra_domain_length": parameters.get(
                    "max_supra_domain_length", self.config.MAX_SUPRA_DOMAIN_LENGTH
                ),
                "correction_method": parameters.get(
                    "multiple_testing_correction", "benjamini_hochberg"
                ),
            }

            logger.info(
                f"Running statistical tests with FDR threshold: {stats_params['fdr_threshold']}"
            )
            ...
            self.statistical_engine = await asyncio.to_thread(
                StatisticalInferenceEngine, protein_domain_map, protein_go_map
            )
    ...
    def _export_associations_tsv(
        self, associations: List[Any], output_file: Path
    ) -> None:
        ...
        try:
            ...
            if start_stage.value <= PipelineStage.DOMAIN_SCANNING.value:
                async with self._progress_context(PipelineStage.DOMAIN_SCANNING):
                    ...
                    if "data_files" not in results and loaded_checkpoint:
                        data_files_meta = loaded_checkpoint.metadata.get(
                            "data_files", {}
                        )
                        results["data_files"] = {
                            k: Path(v) for k, v in data_files_meta.items()
                        }

                    domain_results = await self.run_domain_scanning(
                        parameters, results["data_files"]
                    )
            ...
            if start_stage.value <= PipelineStage.ONTOLOGY_PROCESSING.value:
                async with self._progress_context(PipelineStage.ONTOLOGY_PROCESSING):
                    ontology_results = await self.run_ontology_processing(
                        parameters, results["data_files"]
                    )
                    results["ontology_results"] = ontology_results
                    self.save_checkpoint(
                        PipelineStage.STATISTICAL_INFERENCE, parameters
                    )
            ...
            async with self._progress_context(PipelineStage.CLEANUP):
                ...
                if results.get("inference_results"):
                    ...
                    logger.info(
                        f"Found {stats['significant_associations']} significant "
                ...
                results["run_metadata"] = {
                    ...
                    "parameters": parameters,
                    "resumed_from": loaded_checkpoint.stage.name
                    if loaded_checkpoint
                    else None,
                }
...
@click.option(
    "--force-download", is_flag=True, help="Force re-download of all datasets"
)
@click.option(
    "--datasets",
    multiple=True,
...
@click.option("--disable-async", is_flag=True, help="Disable asynchronous operations")
@click.option(
...
def main(
    ...
    if datasets_list:
        ...
        if invalid_datasets:
            click.echo(
                f"Error: Invalid datasets specified: {invalid_datasets}", err=True
            )
    ...
    try:
        ...
        if "inference_results" in results:
            stats = results["inference_results"]["statistics"]
            click.echo(
                f"Significant associations found: {stats['significant_associations']}"
            )
