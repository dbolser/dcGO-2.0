...
class StatisticalInferenceEngine:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized StatisticalInferenceEngine with {self.total_proteins} proteins"
        )
        logger.info(
            f"Found {len(self.domain_counts)} unique domains and {len(self.go_counts)} unique GO terms"
        )
    ...
    def _validate_inputs(
        ...
        if len(common_proteins) < 10:
            warnings.warn(
                f"Only {len(common_proteins)} proteins with both annotations. "
        ...
        for protein_id, go_terms in protein_go_map.items():
            if not isinstance(go_terms, (set, list)):
                raise TypeError(
                    f"GO terms for protein {protein_id} must be a set or list"
                )
            if not all(isinstance(g, str) for g in go_terms):
                raise TypeError(
                    f"All GO terms for protein {protein_id} must be strings"
                )
    ...
    def _compute_marginals(self) -> None:
        ...
        logger.info(
            f"Domain statistics: {len(self.domain_counts)} unique domains, "
            f"{total_domain_annotations} total annotations"
        )
        logger.info(
            f"GO statistics: {len(self.go_counts)} unique GO terms, "
        ...
        if self.domain_counts:
            ...
            logger.info(
                f"Domain frequency stats - Mean: {domain_freq_stats.mean():.1f}, "
        ...
        if self.go_counts:
            ...
            logger.info(
                f"GO term frequency stats - Mean: {go_freq_stats.mean():.1f}, "
    ...
    def build_correspondence_matrix(self, cache_result: bool = True) -> pd.DataFrame:
        ...
        try:
            ...
            if domain_df.empty or go_df.empty:
                raise RuntimeError(
                    "No valid domain-GO pairs found for matrix construction"
                )

            # Merge on protein to get co-occurrences
            logger.info(
                f"Merging {len(domain_df)} domain pairs with {len(go_df)} GO pairs"
            )
            merged = domain_df.merge(go_df, on="protein", how="inner")

            if merged.empty:
                raise RuntimeError(
                    "No co-occurrences found between domains and GO terms"
                )

            # Create correspondence matrix using crosstab
            correspondence_matrix = (
                pd.crosstab(merged["go_term"], merged["domain"], margins=False)
                .fillna(0)
                .astype(int)
            )

            logger.info(
                f"Built correspondence matrix: {correspondence_matrix.shape[0]} GO terms × "
                f"{correspondence_matrix.shape[1]} domains"
            )
            logger.info(
                f"Matrix density: {(correspondence_matrix > 0).sum().sum()} / "
            ...
            raise RuntimeError(
                f"Correspondence matrix construction failed: {str(e)}"
            ) from e

    def calculate_contingency_values(
        self,
        domain: str,
        go_term: str,
        correspondence_matrix: Optional[pd.DataFrame] = None,
        ...
        if correspondence_matrix is None:
            correspondence_matrix = (
                self.correspondence_matrix or self.build_correspondence_matrix()
            )

        # Get co-occurrence count (a)
        if (
            go_term in correspondence_matrix.index
            and domain in correspondence_matrix.columns
        ):
        ...
        if any(x < 0 for x in [a, b, c, d]):
            raise ValueError(
                f"Invalid contingency table values for {domain}-{go_term}: "
                f"a={a}, b={b}, c={c}, d={d}"
            )
    ...
    def run_statistical_tests(
        ...
        """
        logger.info(
            f"Starting statistical testing with minimum co-occurrence: {min_cooccurrence}"
        )
        ...
        try:
            ...
            logger.info(
                f"Found {len(testable_pairs)} testable domain-GO pairs "
                f"(≥{min_cooccurrence} co-occurrences)"
            )

            if not testable_pairs:
                logger.warning(
                    "No testable pairs found - consider lowering min_cooccurrence threshold"
                )
                return []
            ...
            iterator = (
                tqdm(testable_pairs, desc="Statistical testing")
            ...
            logger.info(
                f"Completed statistical testing for {len(results)} associations"
            )
            ...
            if results:
                ...
                logger.info(
                    f"Found {len(significant_results)} significant associations "
    ...
    def _test_single_association(
        self, domain: str, go_term: str, correspondence_matrix: pd.DataFrame
        ...
        try:
            ...
            a, b, c, d = self.calculate_contingency_values(
                domain, go_term, correspondence_matrix
            )
    ...
    def _apply_fdr_correction(
        self, results: List[AssociationResult], alpha: float = 0.01
    ) -> List[AssociationResult]:
        """
        ...
        logger.info(
            f"  Rejection rate: {100 * len(significant_results) / len(results):.2f}%"
        )
    ...
    def get_summary_statistics(
        self, results: List[AssociationResult]
...
def main():
    ...
    try:
        ...
        for result in results[:5]:  # Show top 5
            logger.info(
                f"  {result.domain} - {result.go_term}: "
