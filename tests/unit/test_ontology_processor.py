...
from ontology_processor import (
    OntologyProcessor,
    Annotation
)
...
class TestOntologyProcessor:
    ...
    def mock_go_graph(self):
        ...
        edges = [
            ('GO:0065007', 'GO:0008150'),  # biological regulation is_a biological process
            ('GO:0050789', 'GO:0065007'),  # regulation of biological process is_a biological regulation
            ('GO:0006355', 'GO:0050789'),  # regulation of transcription is_a regulation of biological process
            ('GO:0045893', 'GO:0006355'),  # positive regulation of transcription is_a regulation of transcription
            ...
            ('GO:0016740', 'GO:0003824'),  # transferase activity is_a catalytic activity
        ]
...
'''
    ...
    def test_optimal_level_filtering_basic(self, ontology_processor):
        ...
        protein_domain_map = {
            f'P{i}': ['PF00001'] for i in range(20)
        }
        protein_go_map = {
            f'P{i}': {'GO:0045893', 'GO:0006355'} for i in range(15)
        }
        protein_go_map.update({
            f'P{i}': {'GO:0006355'} for i in range(15, 20)
        })
        
        with patch.object(ontology_processor, '_test_against_parent_background') as mock_test:
    ...
    def test_parent_background_testing(self, ontology_processor):
        ...
        protein_domain_map = {
            'P1': ['PF00001'], 'P2': ['PF00001'], 'P3': ['PF00002'],
            'P4': ['PF00002'], 'P5': ['PF00003'], 'P6': ['PF00003']
        }
    ...
    def test_annotation_propagation(self, ontology_processor):
        ...
        direct_annotations = [ann for ann in propagated if ann.annotation_type == 'direct']
        assert len(direct_annotations) == len(direct_associations)
        
        # Check that propagated annotations exist
        propagated_annotations = [ann for ann in propagated if ann.annotation_type == 'propagated']
...
class TestOntologyIntegration:
    ...
    def test_complete_ontology_workflow(self, realistic_test_data):
        ...
        mock_associations = [
            AssociationResult('PF00001', 'GO:0045893', 20, 10, 10, 60, 0.001, 8.0, 90.0, 0.005),
            AssociationResult('PF00001', 'GO:0006355', 25, 15, 15, 45, 0.01, 4.0, 70.0, 0.02),
            AssociationResult('PF00002', 'GO:0005515', 30, 20, 20, 30, 0.001, 6.0, 85.0, 0.008),
        ]
        ...
        with tempfile.NamedTemporaryFile(suffix='.obo') as temp_obo:
            with patch('obonet.read_obo') as mock_read:
                ...
                graph = nx.DiGraph()
                graph.add_nodes_from(['GO:0008150', 'GO:0065007', 'GO:0006355', 'GO:0045893', 'GO:0003674', 'GO:0005515'])
                ...
                direct_count = len([a for a in final_annotations if a.annotation_type == 'direct'])
                propagated_count = len([a for a in final_annotations if a.annotation_type == 'propagated'])
                
                assert direct_count > 0
                assert propagated_count > 0
                assert propagated_count > direct_count  # Should have more propagated than direct
