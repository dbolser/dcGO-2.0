...
| --- | --- | ---: | ---: | ---: | ---: | ---: | --- | --- |
| F1 | go (all-domains) — anchor | 92,377 | 3,139,544 | 106,816 | 3.402% | 0.300% | **11.33 [10.89, 11.75]** | provisional — GO is not affected by the Swiss-Prot species defect, but is quoted here as the anchor of a table whose other rows are, and shares all remaining §2 caveats |
| F2 | reactome (in-scope) | 54,685 | 1,487,359 | 2,056 | 0.138% | 0.017% | 7.99 [6.67, 9.55] | provisional — **affected by the species defect** |
| F3 | reactome (all-domains) | 60,202 | 2,309,308 | 2,056 | 0.089% | 0.012% | 7.33 [6.19, 8.75] | provisional — affected |
| F4 | keyword (all-domains) | 85,323 | 2,619,504 | 35,350 | 1.349% | 0.795% | 1.70 [1.64, 1.75] | provisional — affected |
| F5 | subcellular (in-scope) | 13,373 | 507,262 | 7,005 | 1.381% | 0.485% | 2.85 [2.72, 2.97] | provisional — affected |
| F6 | cofactor (in-scope) | 363 | 1,744 | 221 | 12.672% | 3.971% | 3.19 [1.69, 4.03] | provisional — affected |
| F7 | cofactor (all-domains) | 995 | 17,802 | 221 | 1.241% | 0.323% | 3.85 [1.59, 6.17] | provisional — affected |
| F8 | complex (in-scope) | 3,606 | 31,076 | 24 | 0.077% | ~0% | 265.45 [164.55, 455.34] | provisional **and degenerate** — ~1.5 proteins per ComplexPortal entry, so the denominator rounds to zero; magnitude uninterpretable (`VALIDATION_PLAN.md` L391–395) |
| F9 | disease (in-scope / all-domains) | 27 / 44 | 90 / 369 | 0 / 0 | 0% | 0% | `nan` (undefined) | provisional — 0 hits *and* ~0 expected: uninformative, not a negative result |
...
| F11 | Published rounding of F1–F6 as "GO 11.3×, reactome 8.0×, cofactor 3.2×, subcellular 2.9×, keyword 1.7×" | as stated | `VALIDATION_PLAN.md` L371–375; `TODO.md` L49–50; `docs/uniprot_ontology_survey.md` L183–187 | provisional — consistent with F1–F6 to rounding, but inherits the defect |
...
| F14 | The anchor bounds how much the looser truth matters: GO scores 11.33× under the loose protocol vs 12.49× under the strict one | derived from F1 and E2 | `VALIDATION_PLAN.md` L419–420 | provisional — the two figures also differ in candidate set (E2 is supra-domains only, F1 pools all associations), so this is not a clean protocol contrast |
